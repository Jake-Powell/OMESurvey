...
# get_OME_colours2 <- function(n, type = 'contrast'){
#
#   OME_colours = c("#10263B","#66C0D7")
...
get_OME_colours <- function(n, type = 'contrast'){
  ...
  OME_colours = c("#10263B", "#66C0D7" )
  OME_contrast = c('#66C0D7', grDevices::adjustcolor("#10263B", alpha.f = 0.8),  '#D7336C' )
  OME_distinct = c("#009BC1",'#DEB406', '#D7336C',  '#10263B')
  ...
  if(type %in% c('complementary','sequential')){
    ...
    if(n==2) return(OME_colours[1:2])
    if(n > 2){
      rbPal = grDevices::colorRampPalette(OME_colours, alpha = T)
      colours= rbPal(n)
      return(colours)
    }
  }
