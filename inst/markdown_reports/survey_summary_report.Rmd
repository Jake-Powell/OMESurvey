...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    # Tweak df_for_bar to mask out-of-range numeric values to NA for plotting/summaries only
    if (v$type == "numeric" && !is.null(range_spec)) {
      df_for_bar <- df_for_bar |>
        dplyr::mutate(
          "{var}" := dplyr::if_else(
            !is.na(.data[[var]]) &
              (
    ...
    if (!is.null(inv) && length(inv) > 0) {
      ...
      if (!is.null(df_inv)) {
        df_inv |>
          kable_narrow(caption = paste0("Non-allowed responses")) |>
          as.character() |>
          cat()
      }
    ...
    if (is.factor(df_for_bar[[var]])) {
      tab <- janitor::tabyl(df_for_bar[[var]]) |>
      ...
      cat("\n\n Now restrict attention to values filtered by the condition, if applicable. \n\n")
      
      skimr::skim(df_for_bar[[var]]) |>
        kable_narrow() |>
        as.character() |>
        cat()
    }
    ...
    if ((dict |> dplyr::filter(variable_name == var) |> dplyr::pull(data_type)) == "text"){
      ...
      cat("Here are the responses and the grouping variables. There are ",
          ...
          " responses here, from ",
          nrow(data),
          " responses overall. \n\n (Missing/non-response values excluded)\n\n")
    ...
    if ((dict |> dplyr::filter(variable_name == var) |> dplyr::pull(data_type)) == "numeric"){
      ...
      df_for_bar$overall_group <- factor("Overall")
