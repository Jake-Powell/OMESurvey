...
) # if changing see also file type checking in render_survey_summary()
...
required_vars <- c("Variable name", "Final data type", "Allowed values",
                   "Item statement", "report_sec")
...
if (!("grouping_var" %in% names(dict))){
  dict <-
    dict |>
    dplyr::mutate(grouping_var = NA)
}
  ...
  if (length(est_char_vars) > 0){
    ...
    for (i in seq_len(length(est_char_vars))){
      ...
      # first look up x-iles from the establishment characteristics spreadsheet
        ...
        dict <- dict |>
          tibble::add_row(
            `Variable name` = est_char_vars[i],
            `Final data type` = "factor-lo-hi",
            `Allowed values` = paste(final_labels, collapse = ";"),
            `Item statement` = est_char_statements[i],
...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    if ((dict |> dplyr::filter(`Variable name` == var) |> dplyr::pull(`Final data type`)) == "text"){
