...
data <- switch(data_ext,
  ...
  "xlsx" = readxl::read_excel(params$data_path, guess_max = 1e6),
  "xls"  = readxl::read_excel(params$data_path, guess_max = 1e6),
...
if (length(vars_not_in_data) == 0) {
  cat("**Good news**. All variables assigned a section in the data dictionary found in the data.")
} else {
...
if (is.null(params$est_chars_path)){
  cat("**No establishment characteristics.**")
}else{
  ...
  if (length(char_vars_not_in_data) == 0) {
    cat("Good news: All establishment characteristics found in the establishment characteristics data.")
  } else {
    ...
    if (!is.null(cond_expr)) {
      ...
      if (!is.null(cond_mask) && is.logical(cond_mask) && length(cond_mask) == nrow(data)) {
        ...
        if (!is.null(cond_mask)) {
          cat("\n**Condition for variable", var, "did not evaluate to a logical vector of length", nrow(data), ". n_fail_cond, n_meet_cond,  n_meet_cond_missing & n_meet_cond_non_allowed set to NA.**\n\n", sep = " ")
        }
        n_fail_cond <- NA_integer_
        n_meet_cond <- NA_integer_
        n_meet_cond_missing <- NA_integer_
...
parse_report_sec <- function(sec) {
  ...
  m <- stringr::str_match(s, "^([A-Z])([1-9])?:\\s*(.+)$")
  if (!is.na(m[1, 1])) {
    letter <- m[1, 2]
  ...
  stop(
    ...
    "  - 'Xy: Section title' where X is A–Z and y is 1–9"
  )
}
...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    if(v$type == "factor" && !v$allowed_specified){
      cat('\n<span style="color: red;">**WARNING:**</span> no allowed_values specified in the dictionary for this variable. Levels were inferred from the data.\n\n')
    }
