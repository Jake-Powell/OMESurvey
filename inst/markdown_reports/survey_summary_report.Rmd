...
if (!requireNamespace("cowplot", quietly=TRUE)){
  cat(report_notice("Package 'cowplot' is not detected. It is needed to centre legends underneath plots with respect to the whole figure rather than just the plot area. Any section summary plots with long labels on the vertical axis might benefit a bit from  install.packages('cowplot') and then re-running."))
}
```
...
```{r}
data_ext <- tolower(tools::file_ext(params$data_path))

data <- switch(data_ext,
  "csv"  = readr::read_csv(params$data_path, show_col_types = FALSE),
  "xlsx" = OMESurvey::safe_read_excel(params$data_path, guess_max = 1e6),
  "xls"  = OMESurvey::safe_read_excel(params$data_path, guess_max = 1e6),
  stop("Unsupported data file type: ", ext)
) # if changing see also file type checking in render_survey_summary() function


dict <- OMESurvey::safe_read_excel(params$dict_path,
...
Print the dictionary (if verbose)
```{r}
if (params$verbose) {
  dict |>
    # in character columns change newline (many possibilities) to double-space
...
Now deal with any grouping variables from the establishment characteristics. Load the file, check the variables, join relevant variables to the data, warn if anything goes wrong.
      ...
      # first look up x-iles from the establishment characteristics spreadsheet
        if (!exists("est_stats")){
          est_stats <-
            OMESurvey::safe_read_excel(
              params$est_chars_path,
              sheet = switch (params$est_chars_sheet,
...
names(validation_log) <- dict$variable_name
...
for (i in seq_len(nrow(dict))) {
  ...
  if (!is.na(cond_str) && nzchar(cond_str)) {
    ...
    if (!is.null(cond_expr)) {
      ...
      if (!is.null(cond_mask) && is.logical(cond_mask) && length(cond_mask) == nrow(data)) {
        ...
        # count rows that explicitly fail/meet the condition
  ...
  if (!is.null(cond_mask) &&
    ...
    fail_mask <- !cond_mask & !is.na(cond_mask)
  }
  ...
  if (grepl("^factor-", stringr::str_squish(dtype), ignore.case = TRUE)) {
    ...
    if (!is.null(cond_mask) &&
      ...
      n_meet_cond_missing <-
        sum(cond_mask & is.na(data[[var]]), na.rm = TRUE)
      
      n_meet_cond_non_allowed <-
        sum(cond_mask & non_allowed_mask, na.rm = TRUE)
      
    }
    
    # then make a frequency table of non-allowed responses
    non_allowed_freq <- list()
    if (any(non_allowed_mask, na.rm = TRUE)) {
      non_allowed_freq$values <-
        sort(table(raw_chr[non_allowed_mask]), decreasing = TRUE)
    }
    ...
    switch(stringr::str_remove(dtype, "^factor-"),
           `neg-pos`={
             colo <- list(OMESurvey::get_OME_colours(length(allowed), type="contrast"))
             order_values <- list(allowed[seq(from=ceiling(length(allowed)/2)+1, to=length(allowed))])
           },
           `lo-hi`={
             colo <- list(OMESurvey::get_OME_colours(length(allowed), type="complementary"))
             order_values <- list("mean(as.numeric())")
           },
           unordered={
             colo <- list(OMESurvey::get_OME_colours(length(allowed), type="distinct"))
    ...
    if (any(non_allowed_coercion, na.rm = TRUE)) {
      non_allowed_freq_numeric$coercion <-
        sort(table(raw_chr[non_allowed_coercion]), decreasing = TRUE)
    }
    ...
    if (!is.null(out_of_range) && any(out_of_range, na.rm = TRUE)) {
      non_allowed_freq_numeric$out_of_range <-
        sort(table(coerced[out_of_range]), decreasing = TRUE)
    }
    ...
    if (!is.null(cond_mask) &&
        is.logical(cond_mask) &&
        length(cond_mask) == nrow(data)) {
      bad_vals <- data[[var]][cond_mask & non_allowed_mask]
    } else {
...
Print the validation report and the `str()` of the `data` if in verbose mode.
...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    cat("\n\n **Validation info and details of non-allowed responses (all responses)** \n\n")
    ...
    if (v$type == "numeric" && !is.null(range_spec) && v$n_non_allowed > 0) {
      cat(
        "\n*Note:* Numeric values outside the allowed range specified in the data ",
        "dictionary are included in the summaries above ",
        "but excluded from the summary statistics and plots below\n\n",
        sep = ""
      )
    }
    ...
    if (!is.null(cond_expr)) {
      cat("\n\n **Summaries and plots below are restricted to cases where the condition is satisfied.** \n\n")
    }
