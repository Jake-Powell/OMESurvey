...
kable_narrow <- function(x, ..., full_width = FALSE, position = "center") {
  #k <-
  kable(x, ...) |>
    kable_styling(full_width = full_width, position = position)
  #print(k); invisible(k)
}
...
dict <- read_excel(params$dict_path,
                   sheet = params$dict_sheet)


#data <-   read_excel(
#    "C:/Users/pmzdjs/The University of Nottingham/OME - Higher Cohort - Documents/Advanced_years 12 & 13/4. Analysis/Student survey/Cycle 1 2024-25/20251029-OME-Year-12-Student-Survey-2024-25-pseudonymised.xlsx",
#    guess_max = 1e6)
#
#dict  <- read_excel("data_dict_copy.xlsx",
#                    sheet="pupil_survey_Y12")
```

Check variables present in one source and not the other. We probably expect some veriables here, but not too many. Here is the opportunity to check that the variable names the do appear here are ok.

First, those in the dictionary but not in the data:
  ```{r}
setdiff(dict |> pull(`Variable name`),
        data |> names())
```
...
dict <-
  dict |>
  filter(!is.na(report_sec))

# print the dictionary
dict |>
  mutate(across(where(is.character), # in character columns change newline (many possibilities) to double-space
                ~ str_replace_all(., "\\\\r\\\\n|\\r\\n|\\n|\\r", "  "))) |>
  kable()
```
...
if (length(vars_not_in_data) == 0) {
  ...
  dict <-
    dict |>
    filter(!(`Variable name` %in% vars_not_in_data))
...
```{r}
# est_chars <- read_excel(
#     "C:/Users/pmzdjs/OneDrive - The University of Nottingham/OME Research Data - Partner Establishment Characteristics/Partner Establishment Characteristics Pseudonymised Data/2026-01-29-partner_characteristics_june_2025_data.xlsx",
#     sheet = "partner_characteristics_sec")
#
# est_char_vars <-
#   c("TrustCategory",
#     "UrbanRural")
# #est_char_vars <- character(0)
#
# est_char_types <-c(
#   "factor-neg-pos",
#   "factor-unordered"
# )
# est_char_values <- c(
#   "No trust; Trust of 1-9; Trust of 10-19; Trust of 20+",
#   "Conurbation; City or town; Rural"
# )
# est_char_statements <- c(
#   "Trust size",
...
if (is.null(params$est_chars_path)){
  ...
  est_char_vars <- setdiff(est_char_vars, char_vars_not_in_data)

  est_chars <-
    est_chars |>
    select(EstablishmentID, all_of(est_char_vars))
  ...
  if (length(est_char_vars) > 0){
    ...
    data <-
      data |>
      left_join(est_chars |> select(EstablishmentID, all_of(est_char_vars)),
                by=join_by(EST_ID == EstablishmentID))

    # add them to the data dictionary
    for (i in seq_len(length(est_char_vars))){
      dict <-
        dict |>
        add_row(`Variable name` = est_char_vars[i],
...
dict <-
  dict |>
  filter(`Variable name` %in% vars_to_keep)
...
dict <-
  dict |>
  mutate(report_sec = ifelse(is.na(report_sec) | str_trim(report_sec) == "",
                             NA_integer_,
                             report_sec)
         ) |>
  mutate(report_sec_ord = if_else(is.na(report_sec),
                                  NA_integer_,
                                  row_number()
                                  ),
         .by = report_sec)
...
parse_allowed <- function(x){
  if(is.na(x) || str_trim(x) == "") return(NULL)
...
make_validation_row <- function(var, type, colo=list(NA), order_values=list(NA), allowed_specified, allowed,
  ...
  if (length(invalid_mask) == 0) {
    ...
    invalid_examples <- if (length(invalid_values)) list(head(invalid_values, 10)) else list(NA_character_)
  }
...
for (i in seq_len(nrow(dict))) {
  ...
  # coerce using the new coerce_col signature (type, levels)
...
#  factors use allowed; compute invalid_mask from raw_chr
  if (grepl("factor-", dtype, ignore.case = TRUE)) {
    ...
    switch(str_remove(dtype, "^factor-"),
           `neg-pos`={
  ...
  if (!is.null(range_spec)) {
    ...
    out_low <- if (range_spec$lower_incl) coerced < range_spec$lower else coerced <= range_spec$lower
    out_high <- if (range_spec$upper_incl) coerced > range_spec$upper else coerced >= range_spec$upper
    ...
    #invalid_mask[1:5] |> print()
    #invalid_coercion[1:5] |> print()
    #out_of_range[1:5] |> print()

  } else {
    # no numeric range specified: only mark coercion failures as invalid (optional)
...
validation_df <- bind_rows(validation_log)
```
...
for(sec in sections){
  ...
  vars_in_sec <-
    validation_df |>
    filter(report_sec == sec) |>
    arrange(report_sec_ord)

  vars <- vars_in_sec$variable
  if(length(vars) == 0) next

  #if relevant, do the plot:
  if ((vars_in_sec |> select(type, allowed_specified, allowed) |> distinct() |> nrow()) != 1 ||
      length(vars) == 1) { # could use inferred factor values here (for establishment characteristics, for example)?
    ...
    df_plot <- data |> select(any_of(vars))
    ...
    # We know the variables have the same specification, so just get the first one to use
    #colo_vals <- vars_in_sec |> pull(colo) |> unique()
    #colo <- if (length(colo_vals) == 1) colo_vals[[1]] else NA
    colo <- (vars_in_sec |> pull(colo) |> head(n=1))[[1]]

    #colo |> print()

    # if they need ordering then get that too
    order_values <- (vars_in_sec |> pull(order_values) |> head(n=1))[[1]]
    ...
    tryCatch({
      ...
      res <- plot_many_questions(df_plot,
                                 ...
                                 colo=colo,
                                 order_values=order_values,
                                 title_text) #add more here in due course
      if (inherits(res, "ggplot")) {
        ...
        fname <- file.path(fig_dir, sprintf("sec_%s_plot_%s.png", str_extract(sec, "^[^:]+"), "overall"))
        fname <- normalizePath(fname, winslash = "/", mustWork = FALSE)

        # save the plot at the computed size
        ggsave(fname, plot = res, width = 8, height = fig_h, units = "in", dpi = 300)
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    # select grouping (demographic) variables for this report; excluding the variable itself if appropriate
    grouping_vars <-
      dict |>
      filter(grouping_var == TRUE) |>
      pull(`Variable name`) |>
    ...
    # build df for initial_bar: first column = variable of interest, then grouping vars
    if (length(grouping_vars) == 0) {
      if (!is.null(cond_expr)) {
        df_for_bar <- data %>% filter(!!cond_expr) %>% select(all_of(var))
      } else {
        df_for_bar <- data %>% select(all_of(var))
      }
    } else {
      if (!is.null(cond_expr)) {
        df_for_bar <- data %>% filter(!!cond_expr) %>% select(all_of(c(var, grouping_vars)))
      } else {
        df_for_bar <- data %>% select(all_of(c(var, grouping_vars)))
      }
    }
    ...
    #cat("\n\n names of `df_for_bar`")
    #names(df_for_bar) |> print()


    # validation summary and descriptive stats
    v |>
      select(variable, type, colo, allowed_specified, n_invalid,
...
if (dict$`Final data type`[dict$`Variable name` == var] == "chr"){
  ...
  # df_for_bar |> slice_sample(n=10) |> kable_narrow() |> print()
...
# if a numerical variable, do some boxplotting (or density plotting??)
if (dict$`Final data type`[dict$`Variable name` == var] == "numeric"){
  ...
  p_overall <- ggplot(df_for_bar, aes(x=factor(1), y = .data[[num_var]])) +
    #geom_boxplot(width = 0.3, outlier.shape = 21, fill = "lightblue") +
    geom_boxplot() +
    #lvplot::geom_lv() +
    ...
    theme_minimal() +
    coord_flip()
  # p_overall <- ggplot(df_for_bar, aes(x = .data[[num_var]])) +
  #   geom_density() +
  #   labs(#title = paste("Distribution of", num_var),
  #        y = NULL, x = NULL) +
  #   theme_minimal()
  ...
  fname <- file.path(fig_dir, sprintf("sec_%s_var_%02d_plot%02d.png", str_extract(sec, "^[^:]+"), i, 0))
  fname <- normalizePath(fname, winslash = "/", mustWork = FALSE)

  # save the plot at the computed size
  ggsave(fname, plot = p_overall, width = 8, height = 3, units = "in", dpi = 300)
  ...
  for (j in seq_along(group_vars)) {
    ...
    p <- ggplot(df_for_bar, aes(x = .data[[g]], y = .data[[num_var]])) +
      geom_boxplot() +
      #lvplot::geom_lv() +
      labs(#title = paste0(num_var, " by ", g),
        x=g, y=NULL) +
      theme_minimal() +
      coord_flip()

    # p <- ggplot(df_for_bar, aes(x = .data[[num_var]])) +
    #   geom_density() +
    #   facet_grid(rows=vars(.data[[g]])) +
    #   labs(#title = paste0(num_var, " by ", g),
    #        x=NULL, y=g) +
    #   theme_minimal()
    #print(p)
    ...
    # create a unique, stable filename (using section/variable/plot indices)
    fname <- file.path(fig_dir, sprintf("sec_%s_var_%02d_plot%02d.png", str_extract(sec, "^[^:]+"), i, j))
    fname <- normalizePath(fname, winslash = "/", mustWork = FALSE)

    # save the plot at the computed size
    ggsave(fname, plot = p, width = 8, height = 1 + 0.3*nlevels(df_for_bar[[g]]),
           units = "in", dpi = 300)
...
# get colo string from the dict row (adjust column name if different)
#raw_colo <- if ("colo" %in% names(v)) v$colo else dict$colo[dict$`Variable name` == var]
#colo_arg <- if (is.na(raw_colo) || !nzchar(trimws(as.character(raw_colo)))) NULL else raw_colo
...
initial_bar_tryCatch <- function(...) {
  tryCatch({
    ...
    if (inherits(res, "ggplot")) {
      ...
      # save the plot at the computed size
      ggsave(fname, plot = res, width = 8, height = 3, units = "in", dpi = 300)
...
if (is.null(nvars) || nvars == 0) {
  ...
  for (j in seq.int(1, nvars)) {
    ...
    #drop=FALSE to ensure j=1 doesn't become a vector
    initial_bar_tryCatch(sub_df, percCut=percCutVert, colo = colo_arg, fillLabText=NULL, yLabText=NULL)
  }
}
