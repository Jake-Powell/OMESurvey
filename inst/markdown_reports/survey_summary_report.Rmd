...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    if ((dict |> dplyr::filter(`Variable name` == var) |> dplyr::pull(`Final data type`)) == "numeric"){
      ...
      #   ggplot2::labs(y=NULL, x=NULL, title=label) +
      #   ggplot2::theme_minimal() +
      #   ggplot2::coord_flip()
      df_for_bar$overall_group <- factor("Overall")
      p_overall <- OMESurvey::plot_box_with_counts(
        data = df_for_bar,
      ...
      for (j in seq_along(group_vars)) {
        ...
        #   ggplot2::geom_boxplot() +
        #   ggplot2::labs(x=NULL, y=NULL, title=paste0(label,"\n by ", g)) +
        #   ggplot2::theme_minimal() +
        #   ggplot2::coord_flip()
        p <- OMESurvey::plot_box_with_counts(
          data = df_for_bar,
