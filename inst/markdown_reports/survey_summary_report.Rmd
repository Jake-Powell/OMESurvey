...
make_validation_row <- function(var, type, colo=list(NA), order_values=list(NA),
                                ...
                                n_missing, n_invalid, invalid_freq,
  ...
  list(
    ...
    n_missing = n_missing,
    n_invalid = n_invalid,
    invalid_freq = list(invalid_freq),
    condition = condition,
...
for (i in seq_len(nrow(dict))) {
  ...
  if (grepl("^factor-", stringr::str_squish(dtype), ignore.case = TRUE)) {
    ...
    # they use allowed; compute invalid_mask from raw_chr
    ...
    # then make a frequency table of invalid responses
    invalid_freq <- list()
    if (any(invalid_mask, na.rm = TRUE)) {
      invalid_freq$values <-
        sort(table(raw_chr[invalid_mask]), decreasing = TRUE)
    }
    ...
    invalid_freq_numeric <- list()
    
    ## (1) Raw values that failed numeric coercion
    if (any(invalid_coercion, na.rm = TRUE)) {
      invalid_freq_numeric$coercion <-
        sort(table(raw_chr[invalid_coercion]), decreasing = TRUE)
    }
    
    ## (2) Numeric values outside the allowed range
    if (!is.null(out_of_range) && any(out_of_range, na.rm = TRUE)) {
      invalid_freq_numeric$out_of_range <-
        sort(table(coerced[out_of_range]), decreasing = TRUE)
    }
    
    ## Normalise empty case
    if (length(invalid_freq_numeric) == 0) {
      invalid_freq_numeric <- list()
    }
    ...
    validation_log[[var]] <- make_validation_row(
      ...
      n_invalid =
        sum(invalid_coercion, na.rm = TRUE) +
        if (!is.null(out_of_range)) sum(out_of_range, na.rm = TRUE) else 0L,
      invalid_freq = invalid_freq_numeric,
      report_sec = report_sec,
      report_sec_ord = report_sec_ord,
      item_label = item_label,
      condition = if (!is.na(cond_str) && nzchar(cond_str)) cond_str else NA_character_,
    ...
    validation_log[[var]] <- make_validation_row(
      ...
      allowed = NULL,
      n_missing = sum(is.na(data[[var]])),
      n_invalid = NA_integer_, # sum(invalid_mask, na.rm = TRUE),
      invalid_freq = list(),
      report_sec = report_sec,
      report_sec_ord = report_sec_ord,
      item_label = item_label,
  ...
  # message(">>> invalid_mask length and summary")
  # mask <- validation_log[[var]]$invalid_mask
  # if (is.null(mask)) {
  #   message("invalid_mask is NULL")
  # } else {
...
- Here missing means NA or any of the -999, -888, -777 codes
...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    v |>
      dplyr::select(variable, type, colo, allowed_specified, allowed, n_invalid,
    ...
    # Print invalid values (frequency‑sorted, scrollable)
    
    inv <- validation_log[[var]]$invalid_freq[[1]]
    ...
    if (v$type == "numeric" && !is.null(range_spec) && v$n_invalid > 0) {
      cat(
    ...
    if ((dict |> dplyr::filter(variable_name == var) |> dplyr::pull(data_type)) == "text"){
      ...
      miss_tbl <-
        ...
        dplyr::filter(is_missing_text(value)) |>
        dplyr::mutate(
          value = dplyr::coalesce(value, "NA")
        ) |>
        dplyr::count(value, name = "Frequency") |>
        dplyr::arrange(dplyr::desc(Frequency))
      
      if (nrow(miss_tbl) > 0) {
        miss_tbl |>
          kable_narrow(
            caption = "Missing / non-response values"
          ) |>
          as.character() |>
          cat()
      }
      ...
      df_for_bar <-
        df_for_bar |>
        dplyr::filter(!is_missing_text(.data[[var]]))
