...
for(sec in sections){
  ...
  for(i in seq_len(nrow(vars_in_sec))){
    ...
    if (!is.null(inv) && length(inv) > 0) {
      ...
      if (!is.null(df_inv)) {
        df_inv |>
          kable_narrow(caption = paste0("Invalid responses")) |>
          as.character() |>
          cat()
      }
      ...
      if (!is.null(df_inv)) {
        df_inv |>
          kable_narrow(caption = paste0("Invalid numeric responses (could not be parsed)")) |>
          as.character() |>
          cat()
      }
      ...
      if (!is.null(df_inv)) {
        df_inv |>
          kable_narrow(caption = paste0("Invalid numeric responses (out of allowed range)")) |>
          as.character() |>
          cat()
      }
    ...
    if (is.factor(df_for_bar[[var]])) {
      tab <- janitor::tabyl(df_for_bar[[var]]) |>
        ...
        as.data.frame() |>
        rename_NA_first_col("Missing/Invalid") |>
        (\(x) kable_narrow(x,
